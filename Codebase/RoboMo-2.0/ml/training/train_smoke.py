"""
Train the smoke detection classifier on `ml/data/smoke_train.csv`.
The script performs time-aware cross-validation, tunes a probability threshold
to meet a target precision, evaluates a chronological hold-out window, and
exports the final model artifact to `ml/artifacts/smoke_model.joblib`.
"""
...
CSV_PATH = Path("./ml/data/smoke_train.csv")
ARTIFACT_PATH = Path("./ml/artifacts/smoke_model.joblib")
...
def load_dataset(path: Path) -> pd.DataFrame:
    ...
    df = df.dropna(subset=["timestamp", "smoke_present"])
    df = df.sort_values("timestamp").reset_index(drop=True)
    df["smoke_present"] = df["smoke_present"].astype(int)
    return df


def choose_threshold(y_true: np.ndarray, proba: np.ndarray, min_precision: float) -> float:
    precision, recall, thresholds = precision_recall_curve(y_true, proba)
    mask = precision >= min_precision
    if mask.any():
        idx = np.argmax(recall[mask])
        threshold = thresholds[np.where(mask)[0][idx]]
...
def main() -> None:
    ...
    print(f"Loaded {len(df):,} smoke rows from {CSV_PATH}.")

    train_df, holdout_df = split_with_class_coverage(
        df,
        target="smoke_present",
        holdout_fraction=0.15,
        min_class_count=100 if len(df) * 0.15 > 300 else 25,
    )

    X_train = train_df[FEATURES]
    y_train = train_df["smoke_present"].to_numpy()

    X_holdout = holdout_df[FEATURES]
    y_holdout = holdout_df["smoke_present"].to_numpy()
    ...
    threshold = choose_threshold(y_train, oof_proba, min_precision=PRECISION_TARGET)
    ...
    full_X = df[FEATURES]
    full_y = df["smoke_present"].to_numpy()
    ...
    metadata = {
        ...
        "model_version": "v3_precision_prioritized",
    }
